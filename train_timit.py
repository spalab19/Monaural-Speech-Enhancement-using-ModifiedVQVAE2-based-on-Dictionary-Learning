...
class VAETrainer:
    def __init__(self, fftsize, framesize, num_hidden, dim_embedding, num_embedding,
                 batchsize, epoch, learnrate, checkpoint, model_path):
        # init params
        self.fs = 16000
    ...
    def run(self):
        ...
        for i in range(1, self.epoch + 1):
            ...
            plt.ylabel('Loss')
            plt.legend()
            plt.savefig('loss/enhance.png')
            # notify training summery
            elp = (time.time() - self.start) / 3600
            print("Elapsed time:{:.1f}h".format(elp))
            # save model
            torch.save(self.model.state_dict(), "model/enhance_model.pth")

    def _checkpoint(self, epoch):
        checkpoint_path = "checkpoints/{}epoch".format(epoch)
        # plot train loss
        plt.figure(figsize=(10, 6))
        sns.set_style('white')
        sns.set_context('poster')
    ...
    def train(self):
        self.model.train()
        train_loss = 0
        for idx, (imag, mask) in enumerate(self.train_loader):
            imag = imag.to(self.device, dtype=torch.float)
            mask = mask.to(self.device, dtype=torch.float)
            self.optimizer.zero_grad()
            x_tilde, latent_loss = self.model.forward(mask)
            loss = (torch.mean((x_tilde - imag)**2) + torch.mean(latent_loss))
            loss.backward()
            self.optimizer.step()
    ...
    def valid(self, epoch):
        ...
        valid_loss = 0
        with torch.no_grad():
            for idx, (imag, mask) in enumerate(self.valid_loader):
                imag = imag.to(self.device, dtype=torch.float)
                mask = mask.to(self.device, dtype=torch.float)
                x_tilde, latent_loss = self.model.forward(mask)
                loss = (torch.mean((x_tilde - imag)**2) + torch.mean(latent_loss))
                ...
                if (epoch % self.checkpoint==0) and (idx % 5 == 0):
                    ...
                    ax2.imshow(mask.cpu().numpy()[0, 0, :, :]*self.std+self.mean,
                               cmap='inferno', origin='lower', vmin=-63, vmax=33)
                    ax2.axis('off')
                    ax3 = fig.add_subplot(1, 3, 3)
                    ax3.imshow(x_tilde.cpu().numpy()[0, 0, :, :]*self.std+self.mean,
                               cmap='inferno', origin='lower', vmin=-63, vmax=33)
                    ax3.axis('off')
